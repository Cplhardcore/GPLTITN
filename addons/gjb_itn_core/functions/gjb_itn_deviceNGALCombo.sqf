...
//determine if combo light is variable or white-light only
private _wlo = true;
private _bgCtrls = getArray (configFile >> _interface >> "controlsBackground");
_bgCtrls = "knob_modeL" in _bgCtrls;
private _ctrls = getArray (configFile >> _interface >> "controls");
_ctrls = "knob_modeL" in _ctrls;
if(_bgCtrls || _ctrls)then{
	_wlo = false;
};
...
//Functions
		...
		_mode2knob  = {
			...
			private _knobOps1 = ["VAH_co.paa","VAL_co.paa","OFF_co.paa","AL_co.paa","DL_co.paa","AH_co.paa","IH_co.paa","DH_co.paa"];
			private _knobOps2 = ["VIS_co.paa","OFF_co.paa","IR_co.paa"];

			//Map white-light-only device mode onto variable output mode if needed 
			if(_wlo)then{
				switch(_mode)do{
					case 0;
					case 1;
			...
			private _file2 = "";
			switch(_mode)do{
				case 0: {	//VIS AIM HI + IR Flash
					_file1 = _knobOps1 #0;
					_file2 = _knobOps2 #2;
					_comboMode = 1;
				};
				case 1: {	//VIS AIM LO + IR Flash
					_file1 = _knobOps1 #1;
					_file2 = _knobOps2 #2;
					_comboMode = 1;
				};
				case 2: {	//IR AIM LO + Vis Flash
					_file1 = _knobOps1 #3;
					_file2 = _knobOps2 #0;
					_comboMode = 1;
				};
				case 3: {	//IR DUAL LO + Vis Flash
					_file1 = _knobOps1 #4;
					_file2 = _knobOps2 #0;
					_comboMode = 1;
				};
				case 4: {	//IR AIM HI + Vis Flash
					_file1 = _knobOps1 #5;
					_file2 = _knobOps2 #0;
					_comboMode = 1;
				};
				case 5: {	//IR ILLUM HI + Vis Flash
					_file1 = _knobOps1 #6;
					_file2 = _knobOps2 #0;
					_comboMode = 1;
				};
				case 6: {	//IR DUAL HI + Vis Flash
					_file1 = _knobOps1 #7;
					_file2 = _knobOps2 #0;
					_comboMode = 1;
				};
		...
		_interfaceChangeMode = {
			...
			private _knob1 = ["VAH_co.paa","VAL_co.paa","OFF_co.paa","AL_co.paa","DL_co.paa","AH_co.paa","IH_co.paa","DH_co.paa"];
			...
			if(ctrl == 0 && _deviceState #0 > 0)then{
				switch(true)do{
					...
					case (!_simpleCtrl && _newKnob == 2 && _deviceState == 1 && _comboMode == 2);
					case (!_simpleCtrl && _newKnob == 2 && _deviceState == 2);
					case (_simpleCtrl && (_newKnob == 1 || _newKnob == 3)):{
			...
			//Set light mode for white-light-only devices
			if(_wlo)then{
				switch(_comboMode)do{
					case 0: {_newKnob2 = 1};
					case 1: {_newKnob2 = 0};
					case 2: {_newKnob2 = 1};
					case 3: {_newKnob2 = 0};
			...
			switch(true)do{
				case (_newKnob1 == 0 && _newKnob2 == 2): {	//VIS AIM HI + IR Flash
					_newMode = 0;
					_comboMode = 1;
				};
				case (_newKnob1 == 1 && _newKnob2 == 2): {	//VIS AIM LO + IR Flash
					_newMode = 1;
					_comboMode = 1;
				};
				case (_newKnob1 == 3 && _newKnob2 == 0): {	//IR AIM LO + Vis Flash
					_newMode = 2;
					_comboMode = 1;
				};
				case (_newKnob1 == 4 && _newKnob2 == 0): {	//IR DUAL LO + Vis Flash
					_newMode = 3;
					_comboMode = 1;
				};
				case (_newKnob1 == 5 && _newKnob2 == 0): {	//IR AIM HI + Vis Flash
					_newMode = 4;
					_comboMode = 1;
				};
				case (_newKnob1 == 6 && _newKnob2 == 0): {	//IR ILLUM HI + Vis Flash
					_newMode = 5;
					_comboMode = 1;
				};
				case (_newKnob1 == 7 && _newKnob2 == 0): {	//IR DUAL HI + Vis Flash
					_newMode = 6;
					_comboMode = 1;
				};
				case (_newKnob1 == 2 && _newKnob2 == 0): {	//VIS Flash
					_newMode = 7;
					_comboMode = 3;
				...
				case (_newKnob1 == 0 && _newKnob2 == 0): {	//VIS AIM HI + Flash
					_newMode = 8;
					_comboMode = 1;
				};
				case (_newKnob1 == 0 && _newKnob2 == 1):{	//VIS AIM HI
					_newMode = 9;
					_comboMode = 2;
				};
				case (_newKnob1 == 1 && _newKnob2 == 0): {	//VIS AIM LO + Flash
					_newMode = 10;
					_comboMode = 1;
				};
				case (_newKnob1 == 1 && _newKnob2 == 1):{	//VIS AIM LO
					_newMode = 11;
					_comboMode = 2;
				};
				case (_newKnob1 == 2 && _newKnob2 == 1):{	//OFF
					_newMode = 12;
					_comboMode = 0;
				};
				case (_newKnob1 == 2 && _newKnob2 == 2):{	//IR Flash
					_newMode = 13;
					_comboMode = 3;
					if(_deviceState #0 > 0)then{
						_deviceState set [0,_comboMode];
				...
				case (_newKnob1 == 3 && _newKnob2 == 1):{	//IR AIM LO
					_newMode = 14;
					_comboMode = 2;
				};
				case (_newKnob1 == 3 && _newKnob2 == 2):{	//IR AIM LO + Flash
					_newMode = 15;
					_comboMode = 1;
				};
				case (_newKnob1 == 4 && _newKnob2 == 1):{	//IR DUAL LO
					_newMode = 16;
					_comboMode = 2;
				};
				case (_newKnob1 == 4 && _newKnob2 == 2):{	//IR DUAL LO + Flash
					_newMode = 17;
					_comboMode = 1;
				};
				case (_newKnob1 == 5 && _newKnob2 == 1):{	//IR AIM HI
					_newMode = 18;
					_comboMode = 2;
				};
				case (_newKnob1 == 5 && _newKnob2 == 2):{	//IR AIM HI + Flash
					_newMode = 19;
					_comboMode = 1;
				};
				case (_newKnob1 == 6 && _newKnob2 == 1):{	//IR ILLUM HI
					_newMode = 20;
					_comboMode = 2;
				};
				case (_newKnob1 == 6 && _newKnob2 == 2):{	//IR ILLUM HI + Flash
					_newMode = 21;
					_comboMode = 1;
				};
				case (_newKnob1 == 7 && _newKnob2 == 1):{	//IR DUAL HI
					_newMode = 22;
					_comboMode = 2;
				};
				case (_newKnob1 == 7 && _newKnob2 == 2):{	//IR DUAL HI + Flash
					_newMode = 23;
					_comboMode = 1;
				};
			};
			...
			private _onIndicA = _deviceState  #0 > 0 && _deviceState #0 != 3;
			private _onIndicV = !_wlo && !(_deviceState #4 == 7 || _deviceState #4 == 12 || _deviceState #4 == 13);
			private _onIndicW = _wlo && !(_deviceState #4 == 5 || _deviceState #4 == 10);
			if(_onIndicA && (_onIndicV || _onIndicW))then{
				ctrlShow [999,true];
			}else{
				ctrlShow [999,false];
			};
		...
		_interfacePower = {
			...
			switch(_button)do{
				case 1:{
					...
					if(_mode == _off || (!_wlo && _comboMode == 3))exitWith{};
				...
				case 2:{	//For variable head flashlights
					private _mode = _deviceState #4;
					private _off = _deviceCtrls #1 #0;
				...
				case 2.5:{	//For single-output flashlights
					private _mode = _deviceState #4;
					private _off = _deviceCtrls #1 #0;
					//if(_mode == _off || _comboMode == 2)exitWith{};
					...
					switch(true)do{
						case (_mode <= 4):{		// && _comboMode != 2
							//if IR + Flash, then IR
							_mode = _mode +11;
							//_comboMode = 2;
							//_deviceState set [4,_mode];
						};
						case (_mode == 5):{		// && _comboMode isEqualType 0
							//if Flash only, then OFF
							_mode = 10;
							//_deviceState set [4,_mode];
						};
						case (_mode == 10):{		// && _comboMode isEqualType 0
							//if OFF, then Flash only
							_mode = 5 ;
							//_comboMode = 3;
							//_deviceState set [4,_mode];
						};
						case ((_mode == 6 || _mode == 8)):{		// && _comboMode != 2
							//if Vis + Flash, then Vis
							_mode = _mode +1;
							//_comboMode = 2;
							//_deviceState set [4,_mode];
						};
						case ((_mode == 7 || _mode == 9)):{		// && _comboMode != 1
							//if Vis, then Vis + Flash
							_mode = _mode -1;
							//_comboMode = 1;
							//_deviceState set [4,_mode];
						};
						case (_mode >= 11):{		// && _comboMode != 1
							//if IR, then IR + Flash
							_mode = _mode -11;
							//_comboMode = 1;
							//_deviceState set [4,_mode];
...
switch(_ctrl)do{
	...
	case "illumDiff":{	//tap toggle diffuse mode
		private _illumMode = _deviceState #2;
		switch(true)do{
			case (_illumMode == 0);
			case (_illumMode == -1): {
