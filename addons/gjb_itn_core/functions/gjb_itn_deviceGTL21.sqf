...
//Functions
		...
		_initToolTip = {
			...
			switch(_tipMode)do{
				case 0: {
					...
					_pwrbt ctrlSetTooltip (localize "STR_GJB_ITN_CORE_FLPWRALT" + _pwrbtKeys);
					_pwrbt2 ctrlSetTooltip (localize "STR_GJB_ITN_CORE_FLPWRALT" + _pwrbtKeys);
				...
				case 1: {
					_modeSw ctrlSetTooltip _modeStr;
					_pwrbt ctrlSetTooltip localize "STR_GJB_ITN_CORE_FLPWRALT";
					_pwrbt2 ctrlSetTooltip localize "STR_GJB_ITN_CORE_FLPWRALT";
				...
				case 2: {
					...
					} forEach [_modeSw,_pwrbt,_pwrbt2,_illum];
				};
			};
		};
		_changeToolTip = {
		...
		_interfaceChangeMode = {
			...
			private _knob = ["OFF_co.paa","A_co.paa","F_co.paa","D_co.paa"];
			...
			//correct for knob direction on M6X
			if(_interface == "Rsc_gjb_itn_m6x_cup")then{
				switch(_input)do{
					case 0: {
						_input = 1;
					};
			...
			if(_deviceState #0 > 0 && (_newKnob == 0 || _irCheck == 0))then{
		...
		_openInterface = {
			...
			private _knobOps = ["OFF_co.paa","A_co.paa","F_co.paa","D_co.paa"];
